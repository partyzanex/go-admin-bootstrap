...
func (r *authTokenR) GetUser() *User {
	if r == nil {
		return nil
	}
	return r.User
...
var (
	...
	authTokenGeneratedColumns      = []string{}
)
...
func (authTokenL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuthToken interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAuthToken.(*AuthToken)
		if !ok {
			object = new(AuthToken)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAuthToken)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAuthToken))
			}
		}
	} else {
		s, ok := maybeAuthToken.(*[]*AuthToken)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAuthToken)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAuthToken))
			}
		}
	}
...
func AuthTokens(mods ...qm.QueryMod) authTokenQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"goadmin\".\"auth_token\".*"})
	}

	return authTokenQuery{q}
}
...
// Exists checks if the AuthToken row exists.
func (o *AuthToken) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AuthTokenExists(ctx, exec, o.ID)
}
